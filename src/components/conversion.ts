...
export class Conversion implements ConversionInterface {
  ...
  toImperial() {
    ...
    if(this.propertyToConvert === PropertyToConvert.Length) {
        convertedValue =  convertUnits(this.valueToConvert).from('ft').to('m');
    }

    if(this.propertyToConvert === PropertyToConvert.Mass) {
        convertedValue =  convertUnits(this.valueToConvert).from('kg').to('lb');
    }

    if(this.propertyToConvert === PropertyToConvert.Volume) {
        convertedValue =  convertUnits(this.valueToConvert).from('ft3').to('m3');
    }
  ...
  toMetric() {
    let convertedValue =  null;
    if(this.propertyToConvert === PropertyToConvert.Length) {
        convertedValue =  convertUnits(this.valueToConvert).from('m').to('ft');
    }

    if(this.propertyToConvert === PropertyToConvert.Mass) {
        convertedValue =  convertUnits(this.valueToConvert).from('lb').to('kg');
    }

    if(this.propertyToConvert === PropertyToConvert.Volume) {
        convertedValue =  convertUnits(this.valueToConvert).from('m3').to('ft3');
    }
    return convertedValue!.toFixed(this.fixedPoint);
  }
}